...
description: Connect Pi, Oh My Pi (omp), OpenCode, Claude Code, or any MCP client so review comments reach the agent in one click and it replies and resolves them in place.
---
...
| Oh My Pi (omp) | Native | omp is a Pi fork and uses the same bridge core. The bundle is injected as follow-up work into the running session. | Experimental; live-host qualification pending |
...
<Tabs syncKey="agent">
  ...
  <TabItem label="Oh My Pi (omp)">
    <Steps>
      <Step title="Install the extension">
        omp auto-discovers extensions in `~/.omp/agent/extensions/`. Install the package and load its entry as an extension:

        ```sh
        npm install -g @plannotator/artifact-server-omp
        ```

        Add `~/.omp/agent/extensions/artifact-server-omp.ts`:
      ...
      <Step title="Point it at your server">
        A local Artifact Server needs nothing: the extension reads the managed local service's discovery record. For a team server, set two variables before starting omp:
    ...
    <Aside type="note" title="Compatibility">
      omp is a Pi fork. Its `session_shutdown` event does not carry the
      `reason` field Pi's does, so the bridge sends the courtesy disconnect on
      every shutdown rather than only on a confirmed quit — safe because a
      successor session re-registers the same agent row (keyed on hostname and
      working directory). See the integration README.
    </Aside>
  </TabItem>

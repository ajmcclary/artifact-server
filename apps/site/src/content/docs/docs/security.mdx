...
The [CI workflow](https://github.com/ajmcclary/artifact-server/actions/workflows/ci.yml) runs manually or as the reusable release gate. It uses read-only repository permissions, pins third-party actions to complete commit SHAs, does not persist checkout credentials, and installs dependencies from the committed lockfile.
...
The separate [security workflow](https://github.com/ajmcclary/artifact-server/actions/workflows/security.yml) runs manually:
...
See the [Actions history](https://github.com/ajmcclary/artifact-server/actions) for individual runs and logs.
...
Each [GitHub release](https://github.com/ajmcclary/artifact-server/releases) includes package manifests, SPDX software bills of materials, and `SHA256SUMS`. Download all release assets before you verify them.
...
gh attestation verify artifact-server-0.1.1-node.tar.gz \
  --repo ajmcclary/artifact-server
```

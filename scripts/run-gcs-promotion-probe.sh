#!/usr/bin/env bash
# Live GCS sealed-promotion probe (T12): exercises sealed rewrite promotion
# through the GCS blob/staging adapters against a real bucket with a
# bucket-scoped service account. Credentials come from
...
pnpm exec vitest run \
  --config tests/configs/vitest.gcs-promotion-probe.config.ts \
  --reporter=default \
  --reporter=json \
  --outputFile.json=project/evidence/gcs-promotion-probe.json

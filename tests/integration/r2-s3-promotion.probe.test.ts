...
import {createInstallationObjectKeyspace} from
...
import {probeS3SealedPromotion} from
  "../../src/storage/s3-sealed-promotion-probe.js";
...
let promotionSupported: boolean | undefined;

describe("Cloudflare R2 S3 sealed promotion probe", () => {
  test("R2 reports a sealed-promotion precondition verdict and cleans up", async () => {
    const client = createR2Client();
    const installationId = `r2-s3-promotion-probe-${randomUUID()}`;
    const prefix = runPrefix(installationId);
    try {
      promotionSupported = await probeS3SealedPromotion(
        client,
        bucket,
        installationId,
      );
  ...
  test("sealed promotion installs exact bytes and converges create-only", async (context) => {
    if (promotionSupported !== true) {
      context.skip("R2 did not enforce the sealed-promotion preconditions.");
    }
    const client = createR2Client();
    const installationId = `r2-s3-promotion-probe-${randomUUID()}`;
    const prefix = runPrefix(installationId);
  ...
  test("a replaced staged source fails closed and the stream fallback installs", async (context) => {
    if (promotionSupported !== true) {
      context.skip("R2 did not enforce the sealed-promotion preconditions.");
    }
    const client = createR2Client();
  ...
  test("without enforced preconditions the provider factory withholds promote", async (context) => {
    if (promotionSupported !== false) {
      context.skip("R2 enforced the sealed-promotion preconditions.");
    }
    const installationId = `r2-s3-promotion-probe-${randomUUID()}`;
    ...
    const client = createR2Client();
    try {
      await storage.readiness(AbortSignal.timeout(30_000));
      ...
      await removeRunObjects(client, `installations/${namespace}/`);
      await storage.close();
      client.destroy();
    }
  });
...
async function removeRunObjects(client: S3Client, prefix: string): Promise<void> {
  ...
  for (;;) {
    ...
    if ((deleted.Errors ?? []).length > 0) {
      throw new Error("R2 promotion probe cleanup reported object deletion errors.");
    }
  }
  ...
  if ((remaining.Contents ?? []).length !== 0) {
    throw new Error("R2 promotion probe cleanup left run-scoped objects behind.");
  }
}
...
function requiredEnvironment(name: string): string {
  ...
  if (value === undefined || value.length === 0) {
    throw new Error(
      `Run this test through pnpm verify:r2-s3-promotion; ${name} is missing.`,
    );
  }
  return value;
}

/**
 * OMP-LIVE 1 — the full round trip against a REAL omp process.
 *
 * A human publishes an artifact, annotates it, and sends the bundle while omp
 * is mid-work. The suite proves, from the server and from omp's own
 * conversation, that the bundle waits for omp's work boundary, arrives as one
 * message, and is closed by the `artifact_comments` tool until the dispatch
 * reads `addressed`.
 */
...
describe("live omp bridge round trip", () => {
  ...
  beforeAll(async () => {
    ...
    published = (await publishNew(server, installation, {
      ...
      idempotencyKey: "omp-live-round-trip-publish",
      name: "Live round trip report",
    })).body;

    model = await startScriptedModel(async (turn: ModelTurn) => {
      if (turn.index === 1) {
        // The scripted slow work: omp stays inside this unit of work until
        // the test has sent the bundle and watched it reach `delivered`.
        await work.opened;
  ...
  test(
    "OMP-LIVE 1: a bundle sent while omp works arrives at the work boundary and is closed through artifact_comments",
    async () => {
      expect.hasAssertions();

      // The live extension registered this omp session by itself.
      const agent = await waitForConnectedAgent(client);
      expect(agent.displayName).toBe("omp-live-suite");
      ...
      // Put omp to work, and wait until the model is holding that turn open.
      ...
      // Send the bundle while omp is busy.
      const created = dispatchCreationSchema.parse(
        await (await client.sendDispatch({
          agentId: agent.id,
          idempotencyKey: "omp-live-round-trip-dispatch",
      ...
      // The bridge claims it and injects it as follow-up work, all while omp
      // is still inside the first unit of work.
      ...
      // Releasing the work boundary is what lets omp see the bundle.
      work.open();
      ...
      // get_bundle result omp fed back carries both annotation bodies.

/**
 * A scripted, offline model endpoint for the live-OpenCode suite.
 *
 * The suite drives a REAL `opencode` process, which needs a model to run.
 * Instead of a paid provider, this module serves the OpenAI Chat Completions
 * streaming shape from loopback and answers every turn from a planner the test
 * owns. That makes the model deterministic and — more useful — makes it the
 * suite's observation window: every request carries OpenCode's whole
 * conversation, so a test can prove exactly when the bridge's bundle message
 * entered OpenCode's context.
 */
...
/** One text or tool-call message as OpenCode sent it to the model. */
export interface ModelMessage {
  readonly role: string;
  readonly text: string;
}

/** One completion request: OpenCode's whole conversation at that moment. */
...
/** One tool call the scripted model asks OpenCode to run. */
 ...
 * response stream stays open until it resolves, which is how a test holds
 * OpenCode inside a single unit of work while it sends a bundle.
 */
export type ModelPlanner = (turn: ModelTurn) => Promise<ScriptedReply>;

/** The running scripted model. */
export interface ScriptedModel {
  /** Base URL for the OpenCode provider entry, including `/v1`. */
  readonly baseUrl: string;
  stop(): Promise<void>;
  /** Every completion request the live OpenCode process has made, in order. */
...
export async function startScriptedModel(
  ...
  const server: Server = createServer((request, response) => {
    void (async () => {
      ...
      // The opening chunk lands before the planner is consulted, so OpenCode is

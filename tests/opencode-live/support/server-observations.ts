/**
 * Polling reads of the real Artifact Server used by the live-OpenCode suite.
 * Every assertion about what the bridge did lands here, on the HTTP boundary,
 * rather than on scraped terminal text.
 */
...
export async function waitForConnectedAgent(
  ...
  for (;;) {
    ...
    if (Date.now() > deadline) {
      throw new Error("No OpenCode session registered itself as a connected agent.");
    }
    // eslint-disable-next-line no-await-in-loop
